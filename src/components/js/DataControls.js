...
export const DataControls = ({data, display, setDisplay}) => {
    ...
    const CPs = data ? data.map( CP => CP.id.value ) : ["NO DATA"];
    const getCountOptions = key => data ? Array.from( new Set( data[0].counts.map( count => count[key].value ) ) ).sort( sortAlphaNum ) : ["NO DATA"];
    const getCPOptions = key => data ? Array.from( new Set( data.map( CP => CP[key].value ) ) ).sort( sortAlphaNum ) : ["NO DATA"];

    return (
        <div className="HierarchicalGraph_controls">
            <div className="HierarchicalGraph_controls--filterBy">
                <h1>Filter Count Points:</h1>
                <FoldingDropdown title="CP" id="id" operation="Filter" options={CPs} display={display} setDisplay={setDisplay}/>
            ...
            <div className="HierarchicalGraph_controls--viewBy">
                <h1>Filter Counts:</h1>
                <FoldingDropdown title="Year" id="year" operation="Filter" options={getCountOptions( "year" )} display={display} setDisplay={setDisplay}/>
